...
if r is not None:
    ...
    if myJPY["amount"] == myJPY["available"] and myBTC["amount"] == myBTC["available"]:
        if myBTC["amount"] > 0.01:
            raise Exception("TradeController initialize : You have much BTCs but you dont sell them.")
        else:
            thread_flag = buy_jdg
    else:
        raise Exception("TradeController initialize: There are no open order but balance does not match")
    ...
    if thread_flag == buy_jdg:
        ...
        if logic.buy_judge():
            ...
            if result[0]:
                ...
                trader.d_message("TradeController buy_jdg : Failed to send buy signal.")
        ...
        if trade_id[0]==True:
            ...
            if res[0]:
                trade_id[0] = False
                if res[1]=="SELL":
                    trader.d_message("TradeController/buy_order_check : Something strange.")
                    raise Exception("TradeController/buy_order_check : Something strange.")
                elif res[1]=="BUY":
                    amount = res[3]*comm
                    ...
                    if result[0]:
                        ...
                        trade_id[1] = result[1]
                        thread_flag = close_sold_check
                        trader.d_message("You bought BTC successfully and sent sell order\nsize: "+str(amount)+"\nprice: "+str(price))
                    else:
                        trader.d_message("TradeController buy_order_check : Failed to send sell signal.")
                        raise Exception("TradeController buy_order_check : Failed to send sell signal.")
        else:
            trader.d_message("TradeController buy_order_check : Something strange.")
            raise Exception("TradeController buy_order_check : Something strange.")
        ...
        if trade_id[0]==True:
            ...
            if trader.shouldsellall(price):
                ...
                if result_close[0]:
                    ...
                    trader.d_message("!!!!!!!TradeController close_sold_check : Failed to retreat!!!!!!!!!!!.")
                    trader.d_message("!!!!!!!!!!TradeController close_sold_check : Failed to retreat!!!!!!!!!!!.")
                    trader.d_message("!!!!!!!!!TradeController close_sold_check : Failed to retreat!!!!!!!!!!!.")
                    raise Exception("TradeController close_sold_check : Failed to retreat!!!!!!!!!!!.")
        else:
